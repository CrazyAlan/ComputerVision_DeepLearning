...
class KNearestNeighbor:
  ...
  def compute_distances_no_loops(self, X):
    ...
    #########################################################################
    tmp_AB = 2*np.dot(X,self.X_train.T)
    tmp_A2 = sum(X.T**2)
    tmp_B2 = sum(self.X_train.T**2)
    tmp_A2 = np.tile(tmp_A2,(num_train,1)).T
    tmp_B2 = np.tile(tmp_B2,(num_test,1))
    result = tmp_A2 + tmp_B2 - tmp_AB
  ...
  def predict_labels(self, dists, k=1):
    ...
    for i in xrange(num_test):
      ...
      #########################################################################
      tmp = np.argsort(dists[i,:],axis=-1, kind='quicksort', order=None)
      tmp = tmp[0:k]
      ...
      # label.                                                                #
      #########################################################################
      unique,pos = np.unique(closest_y, return_inverse= True)
      counts = np.bincount(pos)
      tmp_idx = np.argsort(counts)[::-1][0:k]
      y_pred[i] = unique[tmp_idx]
      pass
      #########################################################################
      #                           END OF YOUR CODE                            #
      #########################################################################
